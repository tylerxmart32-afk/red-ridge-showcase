...
export function ServiceCard({ slug, icon: Icon, title, description, features }: Service) {
  return (
    <Link
      to="/services/$slug"
      params={{ slug }}
      className="group relative flex h-full flex-col rounded-2xl border border-border bg-surface p-6 shadow-[var(--shadow-card)] transition-all duration-300 hover:-translate-y-1 hover:border-primary hover:bg-accent/50 hover:shadow-[var(--glow-primary)] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
    >
